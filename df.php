...
if ($result->num_rows > 0) {
 echo "<table><tr><th> Flight Number </th><th> Route ID </th><th> Date </th><th> Departure </th><th> Arrival </th><th> Seats </th><th> Price </th></tr>";
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["flightnumber"]. "</td><td> " . $row["routeid"]. "</td><td>". $row["date1"]. "</td><td>". $row["departure"]. "</td><td>". $row["arrival"]. "</td><td>". $row["seats"]. "</td><td>". $row["price"]. "</td><td><form name='delete' action='df1.php' method='POST'><button type='submit' name='delete' value='". $row["flightnumber"]. "' >Delete</button></form></td></tr><br>";
    }
} else {
